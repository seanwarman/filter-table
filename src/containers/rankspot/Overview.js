...
export default class RankOverview extends Component {
    constructor(props) {
        super(props);
    ...
    loadPage = async () => {
        ...
        this.props.changeHeader('sound','Rankspot',[{name: 'Websites', url: '/rankspot/overview'},{name: 'Overview', url: '/rankspot/overview'}]);

        var data = await API.get('biggly', `/rank/key/${this.props.user.apiKey}/site/${localStorage.rank_website_key}/overview/start/${this.state.drStart}`);

        console.log();

        this.setState({graph: data.graph});

        var days = data.graph.length;
        var labelCount = 1;
    ...
    changeWebsite = () => {
        ...
        this.props.history.push('/rankspot/websites');
    ...
    render() {
        ...
        return (
            <Content style={{margin: '24px 16px', padding: 6, minHeight: 280,}}>
                <Row gutter={16}>
                    ...
                    <Col xs={12} style={{marginBottom: 15}}>
                        <div className={"sitePickerDiv"}>
                            ...
                            <Button type="primary" onClick={this.changeWebsite}>
                                {localStorage.rank_website_name}
                            </Button>
                        </div>
                    </Col>
                    <Col xs={12} style={{marginBottom: 15}}>
