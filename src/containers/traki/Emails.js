...
export default class Emails extends Component {
    ...
    loadAnalytics() {
        ...
        return API.get('biggly', `/traki/key/${this.props.user.apiKey}/emails/${localStorage.traki_website_key}/start/${this.state.drStart}/end/${this.state.drEnd}`);

    }

    loadPage = async () => {

        // this.props.changeHeader("Traki Emails");
        this.props.changeHeader('sound','Traki',[{name: 'Traki Emails', url: '/traki/services'}]);
        ...
        this.setState({graphA: data.GraphA, desktop: data.Desktop, mobile: data.Mobile, tablet: data.Tablet, referals: data.Referals, landingPages: data.LandingPages});
...

                                    {
                                        this.state.landingPages.map((item, i) => {
                                            ...
                                            return (
                                                ...
                                                <List.Item key={i}>
                                                    <Col span={20}>
                                                        <span style={{fontSize: '16px'}}>{ item.ck_page }</span>
                                                    </Col>
                                                    <Col span={4} style={{textAlign: 'right', fontWeight: 600}}>
                                                        { item.people }
                                                    </Col>
